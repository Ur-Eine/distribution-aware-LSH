...
int main(int argc, char const* argv[])
{
	...
	float beta = 0.1;
	unsigned Qnum = 100;
	float R_min = 1.0f;//初始半径，和具体数据集有关
	...
	if (argc == 2) {
		...
		if ((argc < 7 || argc > 10)) {//参数个数为7、8、9、10才是正确的传参法（可执行程序本身的argc为1）
		...
		if (argc >= 9){
			Config::trainedWFile = argv[8];
			// Config::gamma = std::sqrt((float)K);
		}
		// if (argc >= 10){
		// 	Config::gamma = std::atof(argv[9]);
		// }
	}
	...
	Parameter param(prep, L, K, R_min);//作用是传递参数至Hash
...
void lshknn(float c, int k, Hash& myslsh, Preprocess& prep, float beta, std::string& datasetName, std::string& data_fold) {
	...
	float mean_time_hash = (float)perform.time_hash / perform.num;
	float mean_time_non_leaf = (float)perform.time_non_leaf / perform.num;
	float mean_time_leaf = (float)perform.time_leaf / perform.num;
	float mean_time_dist = (float)perform.time_dist / perform.num;
	std::cout << "AVG Hash Time:     " << mean_time_hash * 1000 << "ms." << std::endl;
	std::cout << "AVG Non Leaf Time: " << mean_time_non_leaf * 1000 << "ms." << std::endl;
	std::cout << "AVG Leaf Time:     " << mean_time_leaf * 1000 << "ms." << std::endl;
	std::cout << "AVG Dist Time:     " << mean_time_dist * 1000 << "ms." << std::endl;
	...
	if (os) {
		...
		if (tmp == 0) {
			os << "Dataset,c,k,L,K,beta,R_min,RATIO,RECALL,AVG_TIME,COST,DATE" << std::endl;
		}
		std::string dataset = datasetName;
		os << dataset << ',' << c << ',' << k << ',' << myslsh.L << ',' << myslsh.K << ',' << beta << ',' << myslsh.R_min << ','
