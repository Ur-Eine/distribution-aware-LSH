...
public:
	...
	float time_leaf;//在叶子结点上消耗的时间, 包括判断是否加入visit, 距离计算, 更新第k个最小距离的上界
...
struct mbr_compare_less // 比较第一个方块在某个维数上是否小于第二个方块
{
...
template <typename T = float>
struct mbr_compare_center_less //比较第一个方块是否比第二个方块更接近大方块的中心
{
	T **all_mbr; // the array of all MBRs
	int *all_id; // the id of every entry, we need this to ensure same order under unstable sort
...
template <typename T>
T margin(int dim, T *mbr) // 计算对角线的一范数
{
	T sum=(T)0;
...
template <typename T>
double overlap(int dim, T *r1, T *r2) // 计算两个方块重合部分的体积
{
	double sum=(double)1;
	for(int i=0;i<2*dim;i+=2)
	{
		...
		if(lower>=upper) return (double)0; //do not overlap, 若任意一个维数上不重合, 则体积为0
...
template <typename T>
void enlarge(int dim, T *mbr_buf, T *r1, T *r2) // 刚好能包住r1和r2的方块
{
	for(int i=0;i<2*dim;i+=2)
	{
...
struct data_compare_center_less
{
	...
	bool operator()(const int id1, const int id2) const //比较前一个data在特定维数上是否比后一个data更小
	{
...
//T is better to be int, float or double, default is float
template <typename T = float>
class TreeDataR // 树中的方块结点(数据结点有一个数据点, 方块结点有左下和右上两个点)
{
public:
	int dim; // the dimension
	T *data; // contain a list of T
...
public:
	T* nmbr = NULL; // 这个是能刚好覆盖该叶子结点的全部数据的方块
	T maxLengthOfSide = 0; // 这个是方块的最长边长
...
{
	if (visit->termination) return; /*** 此处停止条件, 若 visit 遍历的总点数 ***/
