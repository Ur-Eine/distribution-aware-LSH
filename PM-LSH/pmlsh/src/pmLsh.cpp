...
void pmLsh::improvedSearchWithKth(DataMetric& highData, DataMetric& highQueryData, DataMetric& lowQueryData, DataMetric& lowData2, DataMetric& lowQueryData2, Real_Result& real_result)
{
	/*����*/
	...
	if (outfile && outall) {
		...
		printf("%i\n", lowQueryData.size());
		for (int ij = 0; ij < lowQueryData.size(); ++ij) {
			DATATYPE cost = 0;
			std::priority_queue<knnPair> cand;
			vector<bool> visitFlag(highData.size(), false);
			queryResult[ij].reserve(highData.size());
			...
			while (!myIndex->isCompleteIncrementalRangeSearch(range)) { // 当前代码下 satisfyCount 的值恒为0 (更正, 我已修改 satisfyCount 为正确值)
				myIndex->improvedRangeSearch1(highQueryData[ij], highData, lowQueryData[ij], lowData2, lowQueryData2[ij], range, buffer, cand, cost, visitFlag);
				range *= sqrt(Config::c_appro);
				averageRangeCount++;
			}
			queryResult[ij].resize(Config::KNN);
			int t = Config::KNN - 1;
			while (!cand.empty()) {
				queryResult[ij][t--] = make_pair(cand.top().dist, cand.top().id);
				cand.pop();
			}
			all_cost += cost;
		...
		if (tmp == 0) {
			outfile << "Dataset,c,k,m,beta,M_NUM,PIVOT,Q_NUM,R_MIN,RATIO,RECALL,AVG_TIME,COST,DATE" << std::endl;
		}
		outall.seekp(0, std::ios_base::end);
		tmp = (int)outall.tellp();
		if (tmp == 0) {
			outall << "Dataset,c,k,m,beta,M_NUM,PIVOT,Q_NUM,R_MIN,RATIO,RECALL,AVG_TIME,COST,DATE" << std::endl;
		}
		...
		for (int t = 0; t < 11; t++)
		{
			...
			outData.emplace_back(Config::beta);
