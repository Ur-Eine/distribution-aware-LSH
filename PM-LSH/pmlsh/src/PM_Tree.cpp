...
void PM_Tree::Insert(vector<DATATYPE> feature_val_, int id_)
{
	...
	if (root == nullptr) {	//empty tree
		M_Node_St* new_root = new M_Node_St(nullptr, 1, -1, -1);		//������һ�����ڵ���Ϊ��һ����������ֵ
...
void PM_Tree::improvedRangeSearch1(vector<DATATYPE>& highQuery, DataMetric& highData, vector<DATATYPE>& lowQuery, DataMetric& lowData2, vector<DATATYPE>& lowQuery2,
	DATATYPE searchRange, std::priority_queue<knnPair,std::vector<knnPair>,greater<knnPair>>& buffer, std::priority_queue<knnPair>& res, DATATYPE& all_cost, std::vector<bool>& visitFlag)
{
	...
	while (!buffer.empty() && buffer.top().dist <= searchRange) {
		...
		satisfyCount++; // fix bug
		all_cost++;
		pair0 = buffer.top();
		if (Config::updateDist){
			pair0.dist = MyFunc::Update_Euclidean_distance(lowData2[pair0.id], lowQuery2, pair0.dist);
		}else{
	...
	while (!unVisitNodeQueue->empty()) {
		...
		// 如果当前节点的Lower Bound小于搜索范围
		// 并且搜索的点小于T
		// 并且k个点中最大的距离 * t大于搜索范围
		if (curLB < searchRange 
			&& all_cost <= Config::T&& minDistKth * Config::t > searchRange) { // minDistKth 被初始化为了一个极大值
			// 抛出当前节点
			unVisitNodeQueue->pop();
			// 如果是叶子节点
			if (Is_leaf_Node(curNode)) {
				...
				for (int i = 0; i < curNode->ptr_sub_tree.size(); ++i) {
					...
					if (dist < searchRange) {
						if (Config::updateDist){
							pair0.dist = MyFunc::Update_Euclidean_distance(lowData2[dataNode->id], lowQuery2, dist);
						}else{
							pair0.dist = MyFunc::Cal_Euclidean_distance(highData[dataNode->id], highQuery); // 注意距离函数是开了根号的
						}
						visitFlag[dataNode->id] = true;
						satisfyCount++; // fix bug
						all_cost++;
...
DATATYPE PM_Tree::calLowerBound(vector<DATATYPE>& lowQuery, M_Node_St* node)
{
	...
	for (int i = 0; i < node->hyper_rings.size(); ++i) {
		...
		lb_p_ = max(lb_p, lb_p_);
	}
	return max(lb_o, lb_p_);
}
