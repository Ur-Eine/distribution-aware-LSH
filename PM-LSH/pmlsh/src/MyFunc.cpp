...
DATATYPE MyFunc::Update_Euclidean_distance(const vector<DATATYPE>& a,const vector<DATATYPE>& b,const DATATYPE lb){
	if (a.size() != b.size()) {
		cout << "the size don't match in Update_Euclidean_distance" << endl; 
	}
	DATATYPE sum = lb * lb;
	int m=a.size();
	for (int i = 0; i < m; ++i) {
